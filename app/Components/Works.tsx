...
const Works: React.FC = () => {
  const handleMouseMove = (
    ...
    const rotateX = ((y - centerY) / centerY) * 15; // Increase interaction precision
    const rotateY = ((x - centerX) / centerX) * -15;

    requestAnimationFrame(() => {
      card.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`;
      if (image) {
        image.style.transform = `translateZ(60px) scale(1.15)`; // Faster response
      }
    });
  };
          ...
          >
            ...
            >
              ...
              <Image
                ...
                width={400}
                height={300}
                className="card-image transition-transform duration-150 rounded"
